...
const gradientColor = (text: string) => {
    return text
        .split('')
...
const typewriterEffect = async (text: string, delayTime: number = 5) => {
...
const countdown = async (number: number) => {
...
`);
    const startingMessage = gradientColor
    ('Please log in to get started and track your progress...');
...
const startQuizz = async () => {
    ...
    function Variables(subject: string) {
    let questions:any = [];
